...
def execute(filters=None):
    ...
    if filters.get('view') == 'BWA Kurzbericht':
        ...
        if compare:
            ...
            last_fitlers = {'from_date': get_last_year(filters.get('from_date')),
                            'to_date': get_last_year(filters.get('to_date')), 'compari': 1}
    ...
    if data:
        try:
            data = sorted(data, key=lambda k: (k['sort_zeile'], k['account']))
        except:
            data = sorted(data, key=lambda k: ("zeile" not in k, k.get("zeile", None), "sort_account" not in k, k.get("sort_account", None)))
...
def merge_data(data, last_data):
    for previous_year in last_data:
        ...
        for actual_year in data:
            if actual_year.get("zeilen_name") and actual_year.get("zeilen_name") == previous_year.get("zeilen_name"):
                actual_year["previous_subtotal"] = previous_year.get("subtotal")
                actual_year['absolute'] = actual_year.get('subtotal') - previous_year.get('subtotal')
                added = 1
                percentage = round(((actual_year.get('subtotal') / previous_year.get('subtotal')) * 100.0), 3)
                if percentage > 100:
                    difference = percentage - 100
                else:
                    difference = (100 - percentage) * (-1)
                actual_year['in_percentage'] = round(difference, 2)
            if actual_year.get('account') == previous_year.get('account'):
                if actual_year.get('sum') and previous_year.get('sum'):
                    actual_year['previous_year'] = previous_year.get('sum')
        ...
        if not added:
            data.append({
                'zeile': previous_year.get("zeile"),
                'sort_zeile': previous_year.get("sort_zeile"),
                'sort_name': previous_year.get("sort_name"),
                'account': previous_year.get("account"),
                'name': previous_year.get("name"),
                'previous_year': previous_year.get("sum"),
                'type': previous_year.get("type"),
                'funktion': previous_year.get("funktion")
            })
    return data
    ...
    for previous_year in last_bwa:
        for actual_year in bwa:
            if actual_year.get('zeile') == previous_year.get('zeile'):
                if actual_year.get('subtotal') and previous_year.get('subtotal'):
                    actual_year['previous_subtotal'] = previous_year.get('subtotal')
                    actual_year['absolute'] = actual_year.get('subtotal') - previous_year.get('subtotal')
                    percentage = round(((actual_year.get('subtotal') / previous_year.get('subtotal')) * 100.0), 3)
                    if percentage > 100:
...
def calc_short_bwa(bwa, accounts):
    ...
    for row in accounts:
        if row.get("sum"):
            row["subtotal"] = row.get("sum") if row.get("sum") > 0 else row.get("sum") * (-1)
            row["sum"] = row.get("sum") if row.get("sum") > 0 else row.get("sum") * (-1)
    return accounts
...
def get_row_5440(bwa_row, bwa, accounts):
    res = 0
    for sum in bwa:
        if sum.get('zeile') == bwa_row.get('konto_von') and sum.get('sum'):
            res += sum.get('sum')
        elif sum.get('zeile') == bwa_row.get('konto_von') and sum.get('row_sum'):
            res += sum.get('row_sum')

    for sum in accounts:
        if sum.get('zeile') == bwa_row.get('zeile_von') or sum.get('zeile') == bwa_row.get('zeile_bis') and sum.get(
                'sum'):
            res += sum.get('sum')
        elif sum.get('zeile') == bwa_row.get('zeile_von') or sum.get('zeile') == bwa_row.get('zeile_bis') and sum.get(
                'row_sum'):
            res += sum.get('row_sum')
...
def get_row_addition(r_from, r_to, accounts):
    res = 0
    for sum in accounts:
        if sum.get('zeile') == r_from or sum.get('zeile') == r_to and sum.get('sum'):
            res += sum.get('sum')
        elif sum.get('zeile') == r_from or sum.get('zeile') == r_to and sum.get('row_sum'):
            res += sum.get('row_sum')
    return res
...
def get_span_subtotal(r_from, r_to, accounts):
    res = 0
    for sum in accounts:
        if r_from <= sum.get('zeile') <= r_to and sum.get('sum'):
            res += sum.get('sum')
        elif r_from <= sum.get('zeile') <= r_to and sum.get('row_sum'):
            res += sum.get('row_sum')
...
def get_row_subtotal(row, bwa):
    ...
    for sum in bwa:
        ...
        if sum.get('zeile') == row and sum.get('sum'):
            res += sum.get('sum')
        elif sum.get('zeile') == row and sum.get('row_sum'):
            res += sum.get('row_sum')
    ...
    for elem in sub_rows:
        if elem.get('zeile') not in ordered_subtotal:
            ...
            merged_rows.append({
                ...
                "zeilen": [{"zeile_von": elem.get('zeile_von'), "zeile_bis": elem.get('zeile_bis')}],
                "funktion": elem.get('funktion')
            })
        else:
            for el in merged_rows:
                if el.get("zeile") == elem.get('zeile'):
                    el.get("zeilen").append({"zeile_von": elem.get('zeile_von'), "zeile_bis": elem.get('zeile_bis')})

    ordered_subtotal.sort()

    res = merged_rows + account_totals

    for subtotal in ordered_subtotal:
        for sub in merged_rows:
            sum = 0
...
def get_space():
    sel = """
            ...
            where
                funktion = 9
                and zeile > 1010
                and zeile not in (1094,1331,1350,1390)
            order by
                zeile
            """
...
def get_bwa_account_result(filters):
    ...
    for row in account_rows:
        ...
        if not (row.get('konto_von') and row.get('konto_bis')):
            continue
    ...
    for elem in merged_rows:
        acc_subtotal = []
        row = merged_rows.get(elem)
        for rows in merged_rows.get(elem).get("konto"):
            acc_subtotal += (get_accounts(zeile=elem,
                                          acc_from=rows.get('konto_von'),
                                          acc_to=rows.get('konto_bis'),
        ...
        for subtotal in acc_subtotal:
            ...
            acc_res.append({
                'zeile': elem,
                'sort_zeile': elem,
                'sort_account': subtotal.get('account_number'),
                'sort_name': row.get('zeilen_name'),
